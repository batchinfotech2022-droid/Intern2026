...
namespace InternLAApp.UI.Controllers
{
    public class InternsController : Controller
    {
        ...
        {
            // Retrieve instance, set audit fields and call instance Delete()
            Interns intern = Interns.RetrieveById(id);
            if (intern == null)
            {
                return HttpNotFound();
            ...
            intern.ModifiedBy = usrName;
            intern.ModifiedDate = DateTime.Now;

            bool deleted = intern.Delete();
            if (!deleted)
            {
                // Return the view with an error if delete failed
                ModelState.AddModelError("", "Unable to delete the selected intern.");
                return View(new InternsModel(intern));
            }
